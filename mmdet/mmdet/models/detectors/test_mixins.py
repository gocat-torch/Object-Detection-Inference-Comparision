...
class RPNTestMixin(object):
    ...
    def simple_test_rpn(self, x, img_metas, rpn_test_cfg):
        ...
        # print('rpn out',len(rpn_outs) ,len(rpn_outs[0]))

        # print('rpn out',rpn_outs[0][0].shape,rpn_outs[0][1].shape , rpn_outs[0][2].shape ,rpn_outs[0][3].shape,rpn_outs[0][4].shape) 
        # print('rpn out',rpn_outs[1][0].shape,rpn_outs[1][1].shape , rpn_outs[1][2].shape ,rpn_outs[1][3].shape,rpn_outs[1][4].shape) 
        start = time.time()
        proposal_inputs = rpn_outs + (img_metas, rpn_test_cfg)
        proposal_list = self.rpn_head.get_bboxes(*proposal_inputs)
        print('rpn post-process time',time.time()-start)
        return proposal_list
...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes(self,
        ...
        roi_feats = self.bbox_roi_extractor(
            x[:len(self.bbox_roi_extractor.featmap_strides)], rois)
        print('roi extractor time',time.time()-start)
        ...
        det_bboxes, det_labels = self.bbox_head.get_det_bboxes(
            ...
            rescale=rescale,
            cfg=rcnn_test_cfg)
        print('head post-process time',time.time()-start)
